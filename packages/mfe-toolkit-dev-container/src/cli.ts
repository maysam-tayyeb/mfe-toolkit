...
program
  .name('mfe-dev')
  .description('Run MFE with development container')
  .version('0.1.0');
  ...
  devContainer: {
    ...
    services: {
      ...
      if (!fs.existsSync(distPath)) {
        console.warn(chalk.yellow('⚠️  No dist folder found. Run build first or use --build flag\n'));
      }
      ...
      if (options.hot !== false) {
        const watcher = chokidar.watch([
          path.join(mfePath, 'src'),
          path.join(mfePath, 'dist'),
        ], {
...
module.exports = {
  ...
  devContainer: {
    ...
    sharedDependencies: {
      ${options.framework.includes('react') ? `'react': '^19.0.0',
      'react-dom': '^19.0.0',` : ''}
  ...
  build: {
    ...
    externals: [
      ${options.framework.includes('react') ? `'react',
      'react-dom',` : ''}
