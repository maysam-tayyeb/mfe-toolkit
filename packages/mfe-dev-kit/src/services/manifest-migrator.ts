...
export class ManifestMigrator {
  ...
  migrateRegistry(registry: any): RegistryMigrationResult {
    ...
    const migratedMfes: MFEManifestV2[] = [];
    ...
    for (const manifest of mfes) {
      ...
      if (validation.version === 'v1') {
        // Migrate V1 to V2
        const result = this.migrateManifest(manifest as MFEManifestV1);
        ...
        results.set(manifest.name, {
          success: true,
          manifest: manifest as MFEManifestV2,
        });
        migratedMfes.push(manifest as MFEManifestV2);
      }
    }

    const allSuccess = Array.from(results.values()).every((r) => r.success);

    const newRegistry: MFERegistry = {
  ...
  generateExamples(): Record<string, MFEManifestV2> {
    const basicReactMFE: MFEManifestV2 = {
